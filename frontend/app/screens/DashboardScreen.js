  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [taxRes, foldersRes, docsRes] = await Promise.all([
        taxService.getTaxRecords().catch((err) => {
          console.log('❌ Tax API Error:', err.message);
          return { data: { records: [] } };
        }),
        folderService.getFolders().catch((err) => {
          console.log('❌ Folders API Error:', err.message);
          return { data: { folders: [] } };
        }),
        documentService.getDocuments().catch((err) => {
          console.log('❌ Documents API Error:', err.message);
